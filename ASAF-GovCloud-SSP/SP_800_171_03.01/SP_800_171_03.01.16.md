...
- **Developer workstations (CRMA)**: Personnel access GovCloud via $(System.Collections.Hashtable.Tailscale) over enterprise WiFi. WiFi security: WPA3-Enterprise where available, WPA2-PSK with 20+ character passphrase minimum per $(System.Collections.Hashtable.Policies). No WEP or open networks permitted.
- **Bluetooth**: Disabled on CRMAs connecting to GovCloud per $(System.Collections.Hashtable.Policies) endpoint hardening baseline.
- **Wireless audit**: Covered under $(System.Collections.Hashtable.EndpointAV) network monitoring. Unauthorized WiFi connections alert to Security Hub.
