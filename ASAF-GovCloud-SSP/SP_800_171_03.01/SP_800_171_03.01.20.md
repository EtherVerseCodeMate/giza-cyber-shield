...
- **External system prohibition**: GovCloud CUI data is prohibited from being processed, stored, or transmitted on external systems not covered by an approved security plan. $(System.Collections.Hashtable.SCPDeny) SCP enforces this at the AWS policy layer.
...
- **Personal device prohibition**: $(System.Collections.Hashtable.Policies) Section 6.1 explicitly prohibits access to ASAF GovCloud CUI from personal devices (BYOD), public computers, or shared workstations.
- **Contractor access**: Any future contractor access to GovCloud would require a CRMA provisioned and managed per $(System.Collections.Hashtable.Policies) US Person vetting process.
