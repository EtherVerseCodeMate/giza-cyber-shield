...
Least functionality is implemented â€” only essential capabilities are enabled.

- **Container minimal images**: Iron Bank base images ($(System.Collections.Hashtable.IronBank)) strip unnecessary packages, setuid binaries, and shells from production containers. ECS tasks run as non-root user.
...
- **Disabled by default**: All new AWS resources are deployed with public_access_block=true, enforce_https=true, and encryption enabled â€” secure defaults enforced via Terraform module patterns.
