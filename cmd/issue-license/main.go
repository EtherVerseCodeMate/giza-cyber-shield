...
//
// Real licenses must be signed with the actual root private key, which exists
// only as Shamir shards under keys/root-ceremony/ (see cmd/root-ceremony). You
// need -threshold of those shard files and their passphrases to run this tool.
//
// Usage:
//
...
func main() {
	...
	if *format != "json" && *format != "sacred" {
		logger.Fatalf("invalid -format %q: must be \"json\" or \"sacred\"", *format)
	}
