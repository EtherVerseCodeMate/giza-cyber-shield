...
- 🟢 **Backend is deeper than previously assessed — verified via Supabase dashboard audit.** 89 edge functions deployed. Deployment counts expose what's actually in production use: `stripe-webhook` (108 deployments), `send-password-reset-otp`/`verify-password-reset-otp`/`license-heartbeat`/`license-register` (102–107 deployments each), checkout functions (103). Billing, auth, and license management are battle-tested, not prototype. OSINT pipeline (`osint-orchestrator` 63, `osint-sessions` 64, `osint-tools-list` 65, `github-osint-discovery` 43), GROK AI (`grok-chat` 57, `grok-ai` 36), and PQC key generation (`khepra-key-generator` 55, `cultural-fingerprint-matcher` 62) have real deployment history going back 10 months.
- 🟢 **Database schema is production-grade, not a scaffold.** 35+ tables in `public` schema with RLS enabled on every table. 26 stored functions including `calculate_pqc_readiness()` (scores crypto assets against quantum threat), `has_enterprise_access()` (gates the $499/mo tier), `comprehensive_audit_log` (trigger on all write ops). 9 custom enum types that are unique IP: `agent_trust_level` (untrusted → provisional → verified → trusted → **sentinel**), `adinkra_category` (trust, protection, wisdom, strength, unity, justice, leadership, transformation), `environment_state` (initializing → secured → monitoring → threat_detected → compromised → recovering). Views: `v_pqc_transition_dashboard`, `v_license_health`, `v_security_summary`. Extensions: `pgcrypto` (cryptographic functions in-DB), `uuid-ossp`, `pg_stat_statements`.
- 🟠 **REST API not exposing `public` schema — by config, not accident.** The project's PostgREST is set to expose only the `api` schema. All 35+ tables exist and are queryable via direct DB connection and edge functions using `SUPABASE_SECRET_KEYS`; they simply aren't reachable via the public REST endpoint. This is why invocation calls returned 404 — tables exist, schema gating is deliberate. Fix: expose `public` in Project Settings → API → Exposed schemas (1-click). Not a blocker for Stripe/auth/license flows which go through edge functions.
- 🟠 **Seven secrets have placeholder values — AI features are gated on real keys.** `GROK_API_KEY`, `OPENAI_API_KEY`, `OPENROUTER_API_KEY`, `SHODAN_API_KEY`, `APIVOID_API_KEY`, `STIGVIEWER_API_KEY`, `CLAUDE_MCP_TOKEN` all share the same SHA256 digest (`b0d2e725...`) = identical placeholder value. Any edge function that calls these APIs (OSINT enrichment, AI reasoning, threat intelligence) will silently fail or use an invalid key. Real keys needed before demoing AI-dependent flows. `MAPBOX_PUBLIC_TOKEN` is also empty (SHA256 of empty string).
- 🔴 **Still genuinely unbuilt:** Go-side `pkg/souhimbou/` (Core Agent, SOAR engine) does not exist. KASA threat detector, signed playbook library, RBAC, SOC 2/EU AI Act audit packages — none exist yet. `compliance_control_gaps` and compliance orchestration tables also missing (migration `20260308` not yet applied — blocked by schema conflict, pending resolution). The $99/$499 tiers are priced for a roadmap. What IS built: billing, auth, OSINT, license management, PQC key generation — the operational core of a security platform.
