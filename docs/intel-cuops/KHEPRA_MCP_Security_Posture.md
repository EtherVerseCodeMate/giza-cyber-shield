...
The NSA "Model Context Protocol Security Design Considerations" advisory (2025) identifies four primary attack vectors against AI-serving MCP servers:
...
4. **Prompt storms** — rapid invocation loops exhaust server resources and bypass rate controls
...
|---|---|---|
| Signed tool responses | ML-DSA-65 `SecureEnvelope` + wire-level `_khepra_sig` field (SHA3-256 + HMAC) | `pkg/mcp/chain.go`, `pkg/mcp/router.go` |
...
| Rate limiting + backpressure | Per-agent rate limiter + concurrent call cap (default: 5 concurrent) | `pkg/mcp/validation.go` |
| Human-in-the-loop gate | `godfather_approve` staged report delivery with 30min TTL token | `pkg/mcp/tools/godfather_tools.go` |
| Sandbox isolation | Docker with Seccomp + AppArmor + per-tool capability mounts (CapabilityMounts) | `pkg/mcp/sandbox.go`, `pkg/mcp/security_profiles.go` |
| Manifest pinning | SHA-256 + ML-DSA-65 signed tool manifest, fail-closed on mismatch | `pkg/mcp/manifest.go` |
| Polymorphic provenance | Every request/response wrapped in a provenance envelope | `pkg/mcp/chain.go` |
...
| PQC-signed DAG trail | ML-DSA-65 + SHA3-256 every node, content-addressed immutable graph | `pkg/ea/engine.go`, `pkg/mcp/chain.go` |
...
| **Evolutionarily self-hardening** | EA engine (`pkg/ea/`) continuously evolves security strategy genomes, commits each generation to DAG |
| **Tamper-evident evolution history** | Every EA generation is ML-DSA-65 signed and DAG-committed |
| **NSA/CISA/DISA three-way alignment** | STIG mappings, CMMC practices, and NSA PQC mandate all in one engine |
...
│  │  nist_map  acp_*  nhi_*  khepra_watch                          │    │
│  └───────────────────────────────┬────────────────────────────────┘    │
│                                  │ (optional, isolated)                 │
│  ┌───────────────────────────────▼────────────────────────────────┐    │
│  │  AI Layer (Layer 3 — G0DM0D3)                                  │    │
│  │  Anthropic → OpenRouter → Offline (no external calls)          │    │
│  │  SECURITY INVARIANT: AI calls isolated to pkg/g0dm0d3/         │    │
│  └────────────────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────────────────┘
```
...
*KHEPRA is developed by AdinKhepra / Nouchix. For C3PAO evidence packages, run `godfather_report` with `framework=CMMC-L2`. The resulting report includes the DAG node ID proving the assessment was signed and time-stamped.*
