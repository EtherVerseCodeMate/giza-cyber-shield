...
### Root SCP limit (five policies) — where `DenyRootAccountUsage` lives

AWS Organizations allows **at most five SCPs** to be attached to the **root**; one slot is **`FullAWSAccess`**, leaving **four** org-defined SCPs at root. If you need another deny SCP (e.g. `DenyRootAccountUsage`), a common pattern is to attach it to the **Workloads** OU (or another OU) instead of root.

**Architectural decision (example):** `DenyRootAccountUsage` is attached to the **Workloads** OU (`ou-u7nb-n6kql2ww`) because of the five-policy limit at root. Root-level enforcement is provided by **`FullAWSAccess`** plus the four deny SCPs at root (`DenyNonGovRegions`, `DenyDisableSecurityServices`, `ForceTLS`, `DenyS3PublicAccess` in one deployment). Root account usage is additionally limited by **root MFA**, **no root access keys**, and your **break-glass** procedure.

**Validator config:** list `DenyRootAccountUsage` in **`GOVCLOUD_EXPECTED_SCP_NAMES_WORKLOADS_OU`** (with the correct **`GOVCLOUD_EXPECTED_WORKLOADS_OU_ID`**), not in **`GOVCLOUD_EXPECTED_SCP_NAMES`**, so Step 2 does not expect it at root when it is intentionally attached only on Workloads.

### Checklist inspection — SCP names on an OU

Treat the validator like a **checklist inspection**. You give it a **list of names** you expect to see attached at a **specific Organizational Unit** (the “door”). Step 2 calls `list-policies-for-target` on that OU and compares.

**Rule:** Every SCP **name** in `GOVCLOUD_EXPECTED_SCP_NAMES_WORKLOADS_OU` must appear on **that** OU. If even one expected name is missing → **FAIL**. SCPs attached to the OU that you did **not** list (e.g. `FullAWSAccess`) are fine — extras do not fail the check.

**Your env vars are your promises.** The tool does not infer intent (e.g. “we meant Prod”); it only checks that what you promised for the OU id you supplied matches what AWS returns for that target.

**Example — parent vs child OU:** If you list `DenyNonApprovedServices` and `DenyCUIWithoutUSPersonTag` under the **Workloads** OU id, but those two SCPs are attached only to the **Prod** child OU, the check **fails** — not because AWS is wrong, but because the **door** in your env (Workloads) is not where those policies are attached. **Fix:** Put on the Workloads list only names that `list-policies-for-target` shows **on Workloads** (e.g. `DenyEC2WithPublicIP`, `DenyUnencryptedStorage`, `RequireIMDSv2`, `DenyRootAccountUsage`). For Prod’s list, use the **Prod OU id** as `GOVCLOUD_EXPECTED_WORKLOADS_OU_ID` and set `GOVCLOUD_EXPECTED_SCP_NAMES_WORKLOADS_OU` to the names actually on Prod. (The variable name says “Workloads” for historical reasons; the value is **any OU id** you want to verify in this check.)

**Multiple OUs in one org:** The CLI reads **one** OU id + one comma-separated list per run. To inspect both Workloads and Prod, run Step 2 twice (e.g. swap env in a local script, or two jobs in CI) with the matching id and list each time.
