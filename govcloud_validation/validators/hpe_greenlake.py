"""HPE GreenLake for government workloads — stub (implement when runbook exists)."""
...
class HPEGreenLakeValidator(GovCloudValidator):
    provider_id = "hpe-greenlake"

    STAGES: List[Tuple[str, str]] = [
        ("step_00_prereqs", "0) Pre-Reqs (HPE GreenLake — stub)"),
    ...
    def validate_stage(self, stage_id: str, ctx: ValidationContext) -> StageResult:
        ...
        return StageResult(
            ...
            checks=[
                CheckResult(
                    "stub",
                    "HPE GreenLake validation",
                    CheckStatus.SKIP,
                    "Not implemented — AWS GovCloud first.",
                )
            ],
        )


register("hpe-greenlake", HPEGreenLakeValidator)
