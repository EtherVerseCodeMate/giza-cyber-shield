...
func (e *Engine) execute(t Task) (string, error) {
	...
	// Fallback handler for unclassified task descriptions.
...
func (e *Engine) Chat(message string) string {
	...
	case "PENTEST":
		...
		go func() {
			...
			// Scan results are already recorded in the DAG by RunScan;
			// retrieve them via e.store.All() filtered by the "port-open" action prefix.
