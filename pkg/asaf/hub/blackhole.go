...
func NewBlackholeHandlers(hubAddr string, ebg *policy.EgressBoundaryGuard) *BlackholeHandlers {
	return &BlackholeHandlers{hubAddr: hubAddr, ebg: ebg}
}
...
func (h *BlackholeHandlers) handleEnroll(w http.ResponseWriter, r *http.Request) {
	...
	// Enforce Egress Boundary Guard on reporter IP
	if h.ebg != nil {
		targetIP := req.HostIP
		if targetIP == "" {
			// fallback to actual request IP if not provided in JSON
			if ip, _, err := net.SplitHostPort(r.RemoteAddr); err == nil {
				targetIP = ip
			}
		}
		if err := h.ebg.CheckTarget(r.Context(), targetIP); err != nil {
			writeError(w, http.StatusForbidden, "Egress Boundary Guard blocked enrollment: "+err.Error())
			return
		}
	}
