...
type ToolResult struct {
	...
	SignatureAlgorithm string `json:"signature_algorithm,omitempty"` // e.g. "ML-DSA-65"
}
...
func (s *Server) handleToolsCall(ctx context.Context, req *Request) *Response {
	...
	if len(s.cfg.SigningKey) > 0 && result != nil {
		...
		result.SignatureAlgorithm = "ML-DSA-65"
...
// ─── Inspection Methods (consumed by pkg/mcp/scanner) ─────────────────────────

// Tools returns a copy of the registered tool list.
...
func (s *Server) HasPQCSigning() bool { return len(s.cfg.SigningKey) > 0 }

// HasAuditLogger reports whether a DAG audit logger is wired up.
func (s *Server) HasAuditLogger() bool { return s.cfg.AuditLogger != nil }

// ServerName returns the configured server name.
func (s *Server) ServerName() string { return s.cfg.ServerName }

// SigningKeyLen returns the byte length of the configured signing key (0 if none).
func (s *Server) SigningKeyLen() int { return len(s.cfg.SigningKey) }
