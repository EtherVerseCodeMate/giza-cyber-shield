...
import (
	...
	"github.com/EtherVerseCodeMate/giza-cyber-shield/pkg/flight"
...
// ─── agent_record ─────────────────────────────────────────────────────────────
//
// SouHimBou AI Layer 4→3 bridge: record an agent action in the flight log.
// In sovereign/air-gap mode, writes to the local flight recorder.
// Provides a manual entry point for agents to annotate their own actions.

type AgentRecordResult struct {
	Recorded   bool   `json:"recorded"`
	FrameID    string `json:"frame_id,omitempty"`
	SessionID  string `json:"session_id"`
	Action     string `json:"action"`
	Mode       string `json:"mode"` // "sovereign" | "souhimbou_saas"
...
func HandleAgentRecord(ctx context.Context, call mcp.MCPToolCall) (any, []string, error) {
	...
	if action == "" {
		return nil, nil, fmt.Errorf("agent_record: 'action' is required")
	}
	...
	if toolName != "" {
		intentSummary = fmt.Sprintf("%s (via %s)", action, toolName)
	}

	// Try SouHimBou SaaS endpoint first
	souhimbouEndpoint := os.Getenv("SOUHIMBOU_ENDPOINT")
	mode := "sovereign"
	if souhimbouEndpoint != "" {
		mode = "souhimbou_saas"
		// TODO: forward to SouHimBou AI SaaS via HTTP POST
		// For now: fall through to sovereign mode
	}

	// Sovereign: write to local flight recorder
	dir := os.Getenv("KHEPRA_DATA_DIR")
	if dir == "" {
		dir = "."
	}
	...
	rec, err := flight.New(flight.RecorderConfig{Path: logPath})
	if err != nil {
		return nil, nil, fmt.Errorf("agent_record: open flight recorder: %w", err)
	}
	defer rec.Close()

	frame, err := rec.Record(flight.RecordInput{
		AgentID:       agentID,
		SessionID:     sessionID,
		ToolName:      toolName,
		ToolScope:     "audit:write",
	...
	if err != nil {
		return nil, nil, fmt.Errorf("agent_record: write frame: %w", err)
	}
