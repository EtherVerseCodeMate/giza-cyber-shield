...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    // Send welcome email via Autosend HTTP API
    const autosendRes = await fetch('https://api.autosend.com/v1/mails/send', {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${Deno.env.get('AUTOSEND_API_KEY')}`,
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({
        from: {
          email: Deno.env.get('SMTP_FROM') || 'support@souhimbou.ai',
          name: 'SouHimBou AI',
        },
        to: { email },
        ...
        html: `
          ...
          <body>
            <div class="container">
              ...
              <div class="content">
                <div class="welcome-badge">CLASSIFIED ACCESS</div>
                ...
                <div style="text-align: center;">
                  <a href="https://souhimbou.ai" class="cta-button">
                ...
                <div class="features">
                  <div class="feature">
                    <div class="feature-icon">Threat Intelligence</div>
                  ...
                  <div class="feature">
                    <div class="feature-icon">Live Dashboards</div>
                    <p>Comprehensive operational visibility</p>
                  </div>
                  <div class="feature">
                    <div class="feature-icon">Secure Operations</div>
                ...
                <div class="security-info">
                  <strong>Important:</strong> If you did not request this access or believe this email was sent in error, please contact your security administrator immediately.
                </div>
              </div>
    ...
    if (!autosendRes.ok) {
      const errBody = await autosendRes.text();
      console.error('Autosend API error:', autosendRes.status, errBody);
      throw new Error(`Failed to send welcome email: ${autosendRes.status}`);
    }

    const autosendData = await autosendRes.json();
    ...
    console.log("Welcome email sent successfully:", autosendData);

    return new Response(JSON.stringify({
      success: true,
      messageId: autosendData.id || autosendData.messageId
    }), {
      status: 200,
      headers: {
        "Content-Type": "application/json",
