-- Clear existing generic integrations and add strategic niche-focused integrations
DELETE FROM integrations_library;

-- DoD Contractor Focused Integrations
...
-- DoD TACTICAL & STRATEGIC SYSTEMS
...
-- AI ENTERPRISE & CONTAINER SECURITY
...
-- Add strategic categories to better organize these integrations
UPDATE integrations_library SET category = 'DOD_TACTICAL' 
WHERE provider IN ('DISA', 'US Army', 'NSA', 'DoD CIO');

UPDATE integrations_library SET category = 'INDUSTRIAL_OT' 
WHERE provider IN ('Schneider Electric', 'Siemens', 'Honeywell', 'Rockwell Automation');

UPDATE integrations_library SET category = 'AI_SECURITY' 
WHERE provider IN ('NVIDIA', 'CNCF', 'Databricks', 'Palantir');

UPDATE integrations_library SET category = 'AIR_GAPPED' 
WHERE provider IN ('Raytheon', 'General Dynamics');