-- Drop and recreate auth_type constraint to include strategic authentication methods
ALTER TABLE integrations_library DROP CONSTRAINT IF EXISTS integrations_library_auth_type_check;

-- Add comprehensive auth_type constraint for strategic niches
ALTER TABLE integrations_library ADD CONSTRAINT integrations_library_auth_type_check 
CHECK (auth_type IN (
  -- Existing auth types (compatibility)
  'api_key',
  'basic',
  'oauth2',
  -- Strategic auth types for target niches
  'certificate',        -- DoD PKI/CAC certificates
  'secret_clearance',   -- DoD classified systems
  'tls_mutual',         -- Cross-domain solutions
  'cac_pki',           -- Common Access Card PKI
  'modbus_tcp',        -- Industrial OT protocols
  'iec61850',          -- Power grid protocols
  'honeywell_secure',  -- Proprietary industrial auth
  'ethernet_ip',       -- Industrial Ethernet/IP
  'service_account',   -- Kubernetes service accounts
  'bearer_token',      -- API tokens
  'oauth2_pkce',       -- Enhanced OAuth for sensitive data
  'local_certificate', -- Air-gapped systems
  'two_person_integrity' -- SCIF two-person control
));

-- Now insert the strategic integrations with correct auth types
