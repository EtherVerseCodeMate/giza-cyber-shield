-- STIG-Codex TRL10 Database Schema
...
-- RLS Policies
CREATE POLICY "Organization members can access STIG baselines" ON public.stig_configuration_baselines
  FOR ALL USING (organization_id IN (SELECT get_user_organizations()));

CREATE POLICY "Organization members can access drift events" ON public.stig_drift_events
  FOR ALL USING (organization_id IN (SELECT get_user_organizations()));

CREATE POLICY "Organization members can access monitoring agents" ON public.stig_monitoring_agents
  FOR ALL USING (organization_id IN (SELECT get_user_organizations()));

CREATE POLICY "STIG trusted registry access" ON public.stig_trusted_registry
  FOR ALL USING (organization_id IS NULL OR organization_id IN (SELECT get_user_organizations()));

CREATE POLICY "Organization members can access threat correlations" ON public.stig_threat_correlations
  FOR ALL USING (organization_id IN (SELECT get_user_organizations()));

CREATE POLICY "Organization members can access Open Controls integrations" ON public.open_controls_integrations
  FOR ALL USING (organization_id IN (SELECT get_user_organizations()));

CREATE POLICY "Organization members can access remediation actions" ON public.stig_remediation_actions
  FOR ALL USING (organization_id IN (SELECT get_user_organizations()));
