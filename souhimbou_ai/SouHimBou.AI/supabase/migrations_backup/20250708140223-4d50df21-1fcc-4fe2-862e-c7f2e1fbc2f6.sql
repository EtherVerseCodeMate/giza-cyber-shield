...
CREATE TABLE public.compliance_frameworks (
  ...
  description TEXT,
  category TEXT NOT NULL, -- 'cybersecurity', 'privacy', 'financial', 'healthcare', 'energy'
  authority TEXT, -- 'DoD', 'AICPA', 'ISO', 'PCI SSC', 'HHS', 'NERC'
...
CREATE TABLE public.compliance_controls (
  ...
  control_id TEXT NOT NULL, -- e.g., 'AC.1.001', 'CC6.1', 'A.12.1.1'
  title TEXT NOT NULL,
  description TEXT NOT NULL,
  control_type TEXT NOT NULL, -- 'preventive', 'detective', 'corrective', 'compensating'
  family TEXT, -- 'Access Control', 'Audit and Accountability', etc.
  priority TEXT DEFAULT 'MEDIUM', -- 'CRITICAL', 'HIGH', 'MEDIUM', 'LOW'
  implementation_guidance TEXT,
  assessment_procedures TEXT,
  automation_possible BOOLEAN DEFAULT false,
  automation_query TEXT, -- SQL query for automated checking
...
CREATE TABLE public.compliance_assessments (
  ...
  name TEXT NOT NULL,
  description TEXT,
  assessment_type TEXT NOT NULL, -- 'internal', 'external', 'self', 'continuous'
  status TEXT NOT NULL DEFAULT 'PLANNED', -- 'PLANNED', 'IN_PROGRESS', 'COMPLETED', 'FAILED'
  assessor_name TEXT,
  ...
  compliance_level TEXT, -- Framework-specific compliance level
...
-- Create control assessments table (individual control evaluations)
CREATE TABLE public.control_assessments (
  ...
  status TEXT NOT NULL, -- 'COMPLIANT', 'NON_COMPLIANT', 'PARTIALLY_COMPLIANT', 'NOT_APPLICABLE', 'NOT_TESTED'
  effectiveness TEXT, -- 'EFFECTIVE', 'PARTIALLY_EFFECTIVE', 'INEFFECTIVE', 'UNTESTED'
  implementation_score DECIMAL(5,2), -- 0-100 score
  test_results TEXT,
  findings TEXT,
  recommendations TEXT,
...
CREATE TABLE public.compliance_evidence (
  ...
  evidence_type TEXT NOT NULL, -- 'document', 'screenshot', 'log_export', 'configuration', 'interview'
  title TEXT NOT NULL,
  description TEXT,
  file_path TEXT,
  file_hash TEXT,
  collection_method TEXT, -- 'manual', 'automated', 'system_export'
...
CREATE TABLE public.compliance_exceptions (
  ...
  control_id UUID REFERENCES public.compliance_controls(id) ON DELETE CASCADE,
  assessment_id UUID REFERENCES public.compliance_assessments(id) ON DELETE CASCADE,
  exception_type TEXT NOT NULL, -- 'risk_acceptance', 'compensating_control', 'temporary_deviation'
  title TEXT NOT NULL,
...
CREATE POLICY "Compliance officers can manage controls" 
ON public.compliance_controls 
