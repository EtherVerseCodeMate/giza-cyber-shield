-- Security hardening: Fix critical RLS policy vulnerabilities (corrected version)
...
WITH CHECK (
  ...
  -- Prevent users from modifying security-critical fields by ensuring they remain unchanged
  (CASE WHEN OLD.role IS NULL THEN NEW.role IS NULL ELSE OLD.role = NEW.role END) AND
  (CASE WHEN OLD.master_admin IS NULL THEN NEW.master_admin IS NULL ELSE OLD.master_admin = NEW.master_admin END) AND
  (CASE WHEN OLD.security_clearance IS NULL THEN NEW.security_clearance IS NULL ELSE OLD.security_clearance = NEW.security_clearance END) AND
  (CASE WHEN OLD.plan_type IS NULL THEN NEW.plan_type IS NULL ELSE OLD.plan_type = NEW.plan_type END)
);
