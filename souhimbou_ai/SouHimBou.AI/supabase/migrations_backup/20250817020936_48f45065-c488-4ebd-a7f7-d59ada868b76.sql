-- Security hardening: Fix critical RLS policy vulnerabilities (final version)
-- Only apply changes that don't already exist

-- 1. Harden profiles table RLS policies
-- Drop existing overly permissive policies and recreate them securely
...
-- 2. Add missing RLS policies for subscribers table (only if not exists)
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 4. Enhanced security logging function for sensitive data access
